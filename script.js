...
// Initialize Particles
   ...
   // This will trigger the movement
window.addEventListener('load', () => {
    setTimeout(() => {
        const preloader = document.getElementById('preloader');
        
        // This triggers the CSS 'transform: translateY(-100%)'
        preloader.classList.add('wipe-up');
        
        // Enable scrolling after the preloader has moved out of view
        setTimeout(() => {
            document.body.classList.remove('stop');
        }, 1000); 

    }, 4000); // Keeps the effect visible for 4 seconds
});
...
async function loadPresidentHomeImage() {
    // 1. Select the image inside the glass-card div
    const presidentDiv = document.getElementById('president-img');
    if (!presidentDiv) return;
    
    const imgTag = presidentDiv.querySelector('img');

    // 2. Fetch only the member with the position 'President'
    ...
    if (error) {
        console.error("Error loading President image:", error.message);
        return;
    }

    // 3. If a President exists and has an image, update the src
...
document.addEventListener('DOMContentLoaded', loadPresidentHomeImage);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const observerOptions = {
        ...
        rootMargin: "0px 0px -50px 0px" // Starts slightly before the card hits center
    };
...
// Target Section X for the scroll animation
const sectionX = document.querySelector('.section-x');
if (sectionX) {
    observer.observe(sectionX);
}
...
//event section 
 ...
 // Auto UPDATE MONTH AND YEAR
        ...
        const now = new Date();
        document.getElementById('month-name').innerText = months[now.getMonth()];
        document.getElementById('year-name').innerText = now.getFullYear();

        // 2. Simple Event Data
        const myEvents = [
            { date: "Oct 12", title: "Project Design" },
            { date: "Oct 18", title: "Database Setup" },
            { date: "Oct 25", title: "Core Logic Dev" },
            { date: "Nov 02", title: "System Testing" },
            { date: "Nov 15", title: "Final Submission" },
            { date: "Nov 30", title: "Team Review" }
        ];
...
document.addEventListener("DOMContentLoaded", function() {
    const card = document.querySelector('.glass-card1');
...
async function loadEventsToHome() {
    const eventContainer = document.getElementById('event-list');
    if (!eventContainer) return;

    // Fetch events from Supabase
    const { data: events, error } = await _supabase // Use the variable already holding your key
        .from('events')
        .select('*')
    ...
    events.forEach(event => {
        ...
        // Convert date to Short Month (e.g., "OCT")
        const month = eventDate.toLocaleString('default', { month: 'short' }).toUpperCase();

        const eventItem = document.createElement('div');
        eventItem.className = 'event-item'; 
        eventItem.innerHTML = `
            <div class="event-date-box" style="display: flex; flex-direction: column; align-items: left; justify-content: left; background: rgba(255,255,255,0.1); padding: 5px; border-radius: 8px; min-width: 60px;">
                <span class="day" style="font-weight: bold; font-size: 1.2rem;">${day}  ${month}</span>
               
                 <div class="event-details" style="margin-left: 15px;">
                <h4 style="margin: 0; color: white;">${event.event_name}</h4>
            </div>
            </div>
            `;
...
// Ensure it triggers when the page loads
document.addEventListener('DOMContentLoaded', loadEventsToHome);
